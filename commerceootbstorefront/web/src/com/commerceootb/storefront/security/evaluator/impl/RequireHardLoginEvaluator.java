 ...
 */
package com.commerceootb.storefront.security.evaluator.impl;
...
import com.commerceootb.storefront.security.evaluator.SecurityTraitEvaluator;
			...
			final String guid, final String guidCookieName)
	{
		for (final Cookie cookie : request.getCookies())
		{
			if (guidCookieName.equals(cookie.getName()))
			{
				...
				{
					LOG.info("Found secure cookie with invalid value. expected [" + YSanitizer.sanitize(guid) + "] actual ["
							+ YSanitizer.sanitize(cookie.getValue()) + "]. removing.");
