 ...
 */
package com.commerceootb.storefront.controllers.integration;
...
public class BaseIntegrationController extends AbstractController
{
	...
	protected void initializeSiteFromRequest(final HttpServletRequest httpRequest)
	{
		...
		catch (final MalformedURLException e)
		{
			...
			{
				LOG.debug("Cannot find CMSSite associated with current URL ( " + YSanitizer.sanitize(absoluteURL)
						+ " - check whether this is correct URL) !");
			}
		}
		catch (final CMSItemNotFoundException e)
		{
			LOG.warn("Cannot find CMSSite associated with current URL (" + YSanitizer.sanitize(absoluteURL) + ")!");
			if (LOG.isDebugEnabled())
			{
				LOG.debug(e);
			}
