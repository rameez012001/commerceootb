...
public class AcceleratorAddOnFilter extends GenericFilterBean
{
			...
			ServletException
	{
		...
		{
			LOG.debug("Hit by URI  [" + YSanitizer.sanitize(request.getRequestURI()) + "]");
		}
		if (isActive())
		{
			...
			throws IOException
	{
		...
		{
			LOG.warn("Add-on source file [" + YSanitizer.sanitize(sourceAddOnFileName) + "] should exists ");
			return;
		}
		...
		else
		{
			...
			{
				LOG.info("Copying <<" + YSanitizer.sanitize(sourceAddOnFile.getAbsolutePath()) + ">> to <<"
						+ YSanitizer.sanitize(targetAddOnFile.getAbsolutePath()) + ">>.");
				FileUtils.copyFile(sourceAddOnFile, targetAddOnFile);
			}
		}
	}
