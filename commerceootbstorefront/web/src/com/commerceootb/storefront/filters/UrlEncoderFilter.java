 ...
 */
package com.commerceootb.storefront.filters;
...
import com.commerceootb.storefront.web.wrappers.UrlEncodeHttpRequestWrapper;
...
public class UrlEncoderFilter extends OncePerRequestFilter
{
			...
			final FilterChain filterChain) throws ServletException, IOException
	{
		...
		{
			LOG.debug(" The incoming URL : [" + YSanitizer.sanitize(request.getRequestURL().toString()) + "]");
		}
		...
		if (currentUrlEncoderDatas != null && !currentUrlEncoderDatas.isEmpty())
		{
			...
			if (LOG.isDebugEnabled())
			{
				LOG.debug("ContextPath=[" + YSanitizer.sanitize(wrappedRequest.getContextPath()) + "]" + " Servlet Path= ["
						+ YSanitizer.sanitize(wrappedRequest.getServletPath()) + "]" + " Request Url= ["
						+ YSanitizer.sanitize(wrappedRequest.getRequestURL().toString()) + "]");
			}
