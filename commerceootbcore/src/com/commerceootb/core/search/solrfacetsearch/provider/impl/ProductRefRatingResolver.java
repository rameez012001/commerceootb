...
public class ProductRefRatingResolver extends ModelAttributesValueResolver<ProductModel> {
    @Override
    protected Object getAttributeValue(IndexedProperty indexedProperty, ProductModel model, String attributeName) throws FieldValueProviderException {
        ...
        return productReferences.stream().filter(ref->ref.getReferenceType().equals(ProductReferenceTypeEnum.BUNDLE)).map(ref-> ref.getTarget().getAverageRating()!=null?ref.getTarget().getAverageRating():0.0d).toList();
    }
}
