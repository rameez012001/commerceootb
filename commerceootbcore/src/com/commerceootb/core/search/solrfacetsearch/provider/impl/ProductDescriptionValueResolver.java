...
public class ProductDescriptionValueResolver extends ModelAttributesValueResolver<ProductModel> {
    @Override
    protected Object getAttributeValue(IndexedProperty indexedProperty, ProductModel model, String attributeName) throws FieldValueProviderException {
        ...
        return productReferences.stream().filter(ref->ref.getReferenceType().equals(ProductReferenceTypeEnum.BUNDLE)).map(ref->ref.getTarget().getDescription()).toList();
    }
}
